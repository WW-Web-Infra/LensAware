...
enum HealthDetectionState: Equatable {
    case idle
    case analyzing
    case responded(HealthAnalysisResponse)
    case failed(String)
...
final class HealthDetectionManager: ObservableObject {
    ...
    @Published private(set) var lastResponse: HealthAnalysisResponse?
    ...
    init() {
        do {
            claudeService = try ClaudeVisionService()
        } catch {
            claudeService = nil
...
extension HealthDetectionManager: CameraFrameDelegate {
    ...
    private func processFrame(_ imageData: Data) async {
        ...
        guard case .idle = detectionState else { return }  // drop frame if already analyzing
        ...
        do {
            ...
            // Persist asynchronously — don't block the detection pipeline
            ...
            detectionState = .failed(error.localizedDescription)
            // Auto-recover to idle after a short delay so the next frame can retry
            Task {
                try? await Task.sleep(nanoseconds: 5_000_000_000)
                if case .failed = detectionState {
                    detectionState = .idle
                }
            }
        }
